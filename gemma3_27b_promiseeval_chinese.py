...
- Prompts in English (as per your previous setup)
- Logic synced with Chinese Master Thesis script
...
OUT_DIR = r"C:\Users\lubob\Desktop\master thesis\results\Gemma3_27b_PromiseEval_Chinese"
...
def get_sample_content(sample: Dict[str, Any]) -> str:
    """
    同步中文版邏輯：優先抓取 promise_string，其次抓取 evidence_string
    """
...
async def call_ollama_async(prompt: str, rate_limiter: AsyncRateLimiter) -> Dict[str, Any]:
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            payload = {"model": MODEL, "prompt": prompt, "format": "json", "stream": False, "options": {"temperature": TEMPERATURE, "num_predict": 100}}
...
def _normalize_pred_single(task: str, enums: List[str], d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # 簡單映射
    mapping = {"yes": "Yes", "no": "No", "n/a": "N/A", "na": "N/A", "clear": "Clear", "unclear": "Not Clear"}
    norm = mapping.get(val.lower(), val)
    if norm not in enums:
        norm = "N/A" if task in ("evidence_quality", "verification_timeline") else "No"
    return {task: norm}
...
async def process_single_sample(sample: Dict[str, Any], idx: int, task: str, cfg: Dict[str, Any], retriever: RAGRetriever, rate_limiter: AsyncRateLimiter) -> Dict[str, Any]:
    try:
        ...
        if not input_text: input_text = sample.get("text", "")
        
        sample_copy = sample.copy()
        sample_copy["text"] = input_text
        
        sys_prompt = cfg["rag_system_prompt"] if (USE_RAG and retriever) else cfg["no_rag_system_prompt"]
        prompt = build_messages_for(sample_copy, sys_prompt, retriever, task)
        raw_pred = await call_ollama_async(prompt, rate_limiter)
        return {"idx": idx, "input": {"text": input_text}, "pred": _normalize_pred_single(task, cfg["enums"], raw_pred)}
    except Exception as e:
...
async def run_one_task_async(task: str, samples: List[Dict[str, Any]], out_dir: str, retriever: RAGRetriever = None):
    ...
    out_path = pathlib.Path(out_dir) / f"chinese_ollama_{task}_predictions.jsonl"
...
async def main():
    global USE_RAG
    print(f"🚀 Starting Chinese ESG Prediction with {MODEL}")
    
    # 修改點：處理中文資料集的嵌套結構
    raw_test = load_samples(TEST_PATH)
    test_samples = raw_test.get("chinese_test", raw_test) if isinstance(raw_test, dict) else raw_test
    print(f"[INFO] Loaded {len(test_samples)} samples from Chinese Test Set")
    ...
    if USE_RAG:
        ...
        if pathlib.Path(TRAIN_PATH).exists():
            ...
            train_samples = raw_train.get("chinese_train", raw_train) if isinstance(raw_train, dict) else raw_train
            print(f"[INFO] Loaded {len(train_samples)} samples from Chinese Train Set")
